...
Parser.prototype.write = function (chunk) {
    ...
    while (c = chunk.charAt(i++)) {
        ...
        case S.OPEN_CHAR:
            ...
            else if ((c === '?' || c === '^') && this._tags[c]) {
                this._tagName = c;
                this._attributeName = 'cond';
                this._state = S.OPEN_COND_TAG;
            } else {
        ...
        case S.OPEN_NESTED_CHAR:
            break;

        case S.OPEN_NESTED_TAG:
            if (this._is(Parser.CharSet.ALPHANUM, c)) {
                this._nestedTag += c;

            } else if (this._is(Parser.CharSet.WHITESPACE, c) || c === '}') {
                this._nests.push(this._nestedTag);
                if (this._nestedTag === '?' || this._nestedTag === '^'){
                    throw new Error('Malformed tag. Tag not closed correctly.');
                }
                else {
                    this._attributeValue += '{@' + this._nestedTag + c;
                }
                this._nestedTag = '';
                this._state = S.ATTRIB;

            } else if (c === '/') {
                if(this._nestedTag === '?' || this._nestedTag === '^' || this._nestedTag === 'if'){
                    throw new Error('Malformed tag. Tag not closed correctly.');
                }
                else {
                    this._attributeValue += '{@' + this._nestedTag + c;
                    this._nestedTag = '';
                    this._state = S.COND_RESULT;
                }
        ...
        case S.OPEN_COND_TAG:
            if (this._is(Parser.CharSet.ALPHANUM, c)) {
                this._attributeValue += c;

            } else if (this._is(Parser.CharSet.WHITESPACE, c) || c === '/') {
                this._textNode += '{' + this._tagName + this._attributeValue + c;
                this._state = S.TEXT;
        ...
        case S.COND_RESULT:
            if (c === '{') {
                this._state = S.NEXT_COND_TAG;
            } else {
                this._attributeValue += c;
            }
            break;

        case S.NEXT_COND_TAG:
            if (c === '}') {
                // open a new nested conditional tag
                if(this._followupTag[0] === '?' || this._followupTag[0] === '^') {
                    this._attributeValue += '{'+this._followupTag+c;
                    this._nests.push(this._followupTag.slice(1));
                    this._followupTag = '';
                    this._state = S.COND_RESULT;
                }
                // open a new nested if
                else if (this._followupTag.indexOf('@if ') === 0) {
                    this._attributeValue += '{'+this._followupTag+c;
                    this._nests.push('if');
                    this._followupTag = '';
                    this._state = S.COND_RESULT;
                }
                // close a nested conditional tag
                else if(this._followupTag === '/'+this._nests[this._nests.length-1]){
                    this._nests.pop();
                    this._attributeValue += '{'+this._followupTag+c;
                    this._followupTag = '';
                    this._state = S.COND_RESULT;
                }
                // else tag of this conditional tag
                else if(this._nests.length === 0 && this._followupTag === ':else') {
                    this._followupTag = '';
                    this._attributes[this._attributeName] = this._attributeValue;
                    this._attributeValue = '';
                    this._attributeName = 'else';
                    this._state = S.COND_RESULT;
                }
                else if(this._nests.length === 0 && this._tagName === 'if' && this._followupTag === '/if') {
                    this._followupTag = '';
                    this._attributes[this._attributeName] = this._attributeValue;
                    this._closeTag();
                    this._state = S.TEXT; 
                }
                // close this conditional tag
                else if(this._nests.length === 0 && this._attributes.cond && this._followupTag === '/'+this._attributes.cond) {
                    this._followupTag = '';
                    this._attributes[this._attributeName] = this._attributeValue;
                    this._closeTag();
                    this._state = S.TEXT;
                }
                // treat as text within result
                else{
                    this._attributeValue += '{'+this._followupTag+c;
                    this._followupTag = '';
                    this._nestedLevel++;
        ...
        case S.ATTRIB:
            ...
            } else if (c === '}' && this._tagName.indexOf('if') === 0) {
                this._attributeName = 'if';
                this._attributeValue = '';
                this._state = S.COND_RESULT;
            } else {
        ...
        case S.ATTRIB_VALUE:
            if (c === '/') {
                ...
                throw new Error('Malformed tag. Attribute values must use double quotes.');
